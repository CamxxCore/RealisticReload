...
BOOL Utility::PlaySoundResource(int id, BOOL async)
{
	BOOL bRtn;
	HANDLE hRes;

	auto hInst = GetActiveModule();

	HRSRC hResInfo = FindResource(hInst, MAKEINTRESOURCE(id), "WAVE");
	...
	if (lpRes != NULL) {
		bRtn = sndPlaySound(lpRes, 
			SND_MEMORY | async | SND_NODEFAULT);
